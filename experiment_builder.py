...
class ExperimentBuilder(object):
    ...
    def evaluated_test_set_using_the_best_models(self, top_n_models):
        ...
        print(per_batch_targets.shape)
        print(per_batch_max.shape)
        per_batch_targets_list = per_batch_targets.flatten()
        per_batch_max_list = per_batch_max.flatten()
        print(per_batch_targets_list.shape)
        print(per_batch_max_list.shape)
        accuracy = np.mean(np.equal(per_batch_targets, per_batch_max))
        accuracy_std = np.std(np.equal(per_batch_targets, per_batch_max))
        np.savetxt("per_batch_targets_random.csv", per_batch_targets, delimiter=",")
        np.savetxt("per_batch_max_random.csv", per_batch_max, delimiter=",")
        f1 = f1_score(y_true=per_batch_targets_list, y_pred=per_batch_max_list, average='macro')  # 也可以指定micro模式
        precision = precision_score(y_true=per_batch_targets_list, y_pred=per_batch_max_list, average='macro')
        recall = recall_score(y_true=per_batch_targets_list, y_pred=per_batch_max_list, average='macro')  # 也可以指定micro模式
